...
/**
 * Keys of `tagInputProps` that are still accepted for source compatibility with the former
 * BlueprintJS `TagInput` but currently have no effect. They are slated for removal in v27.
 */
const NO_OP_TAG_INPUT_PROPS = [
    "addOnBlur",
    "addOnPaste",
    "autoResize",
    "inputValue",
...
// Track which no-op prop names have already been warned about, so the dev-mode warning fires
// only once per prop name (across all instances) instead of on every render.
const warnedNoOpTagInputProps = new Set<string>();

/** Warns (once per prop name, dev builds only) when a no-op `tagInputProps` key is passed. */
const warnNoOpTagInputProps = (tagInputProps: MultiSuggestFieldTagInputProps | undefined): void => {
    if (process.env.NODE_ENV === "production" || !tagInputProps) {
        return;
    }
    for (const propName of NO_OP_TAG_INPUT_PROPS) {
        if (tagInputProps[propName] !== undefined && !warnedNoOpTagInputProps.has(propName)) {
            warnedNoOpTagInputProps.add(propName);

            console.warn(
                `MultiSuggestField: \`tagInputProps.${propName}\` currently has no effect and is slated ` +
                    `for removal in v27. Remove it to avoid relying on behavior that is not implemented.`,
            );
        }
    }
};
...
export function MultiSuggestField<T>({
    ...
    // Dev-mode notice for `tagInputProps` keys that are accepted but currently have no effect.
    React.useEffect(() => {
        warnNoOpTagInputProps(tagInputProps);
    }, [tagInputProps]);
                ...
                >
                    <div
                        className={cn(
                            ...
                            "flex min-h-8 w-full items-center gap-1 rounded-lg border border-input bg-transparent px-2.5 py-1 text-sm",
