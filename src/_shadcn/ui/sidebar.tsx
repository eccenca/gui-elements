/**
 * Vendored shadcn/ui `sidebar` (style: new-york-v4).
 * Local adaptations: `cn` import path, `@radix-ui/react-slot` in place of the `Slot` export
 * from the `radix-ui` meta package, local `useIsMobile` hook, local sibling primitives,
 * `React.forwardRef` re-added (React 18 — registry code relies on React-19 ref-as-prop).
 * The mobile sheet keeps this library's `SheetContent` close button instead of hiding it
 * via the registry's `[&>button]:hidden` selector.
 */
